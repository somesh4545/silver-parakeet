...
export default function Home({ params }) {
  const [course_id, setcourse_id] = useState(params.slug);
  ...
  const fetchCourse = async () => {
    await fetch("/api/course-detail?course_id=" + course_id)
      .then((response) => response.json())
      .then((data) => {
        setCourseContent(data);
        setActiveContent(data.sections[0].subsections[0]._id);
        setprocessing(false);
      })
  ...
  return (
    <main className=" min-h-screen bg-zinc-950 flex flex-col">
      ...
      {processing == true ? (<div className="flex justify-center items-center">
        ...
        <div className="min-h-full flex-grow flex">
          <div className="p-8 w-3/4 min-h-full flex flex-grow flex-col">
            <VideoRenderer data={courseContent} activeContent={activeContent} />
            <VideoInformation
              data={courseContent}
          ...
          <div className="w-1/4 bg-zinc-900 min-h-full flex flex-grow flex-col">
            <CourseContent
              activeContent={activeContent}
